...
<body>
   ...
   // pre($videoType);die;
  if($videoType == 'epg'){
   $content_details['data']['show_id'] = $content_details['data']['id']??'';
   $content_details['data']['channel_name'] = $content_details['data']['title']??'';
  }
   $time = time();
   $end = isset($content_details['data']['end'])?$content_details['data']['end']:'';
   $video_types = '/Video';
   if($time < $end){
   $video_types = "/Live";   
   }
   $title_video = isset($content_details['data']['show_id']) 
   ? $content_details['data']['show_id'] . '/' . (isset($content_details['data']['channel_name']) ? $content_details['data']['channel_name'] : '') 
   : '';

 
 if ((isset($content_details['status']) && $content_details['status'] == true && !empty($content_details['data'])) || (isset($file_url) && $file_url) || ((isset($drm_file_url) && $drm_file_url) && (isset($drm_token) && $drm_token))) { 
//$matomo_title =  $content_details['data']['id']."/".$content_details['data']['title'];?>
      ...
      <section class="pt-3 pb-5">
               ...
               <div class="col-lg-12 mx-auto">
                  <div class="">
                     <div class="row m-0">
                        ...
                        $descriptions = '';
                        if (isset($content_details['data']['description']) && is_array($content_details['data']['description'])) {
                        ...
                        $timeDuration = '';
                        if(isset($content_details['data']['end']) && !empty($content_details['data']['end'])){
                           $differenceInSeconds = $content_details['data']['end'] - $content_details['data']['start'];
                        ...
                        <div class="py-3 col-md-8">
                           ...
                           <div class="pt-3">
                              ...
                              <?php if(isset($content_details['data']['start']) && !empty($content_details['data']['start'])){ ?>
                                 <p class="line21 tex-fo-500 text-white"><?= date('D, d F', $content_details['data']['start']) ?> <span class="dot_upcoming mx-2"></span> <span><?= date('h:i A', $content_details['data']['start']) ?></span>
                              <?php } ?>
                              <?php if(!empty($timeDuration)){ ?>
                                 <!-- <span class="dot_upcoming mx-2"></span><span><?//= $timeDuration ?></span> -->
                              <?php } ?>
                              <?php if(!empty($content_details['data']['genres'])){ 
                                 if(isset($content_details['data']['start']) && !empty($content_details['data']['start'])){ ?>
                                 <span class="dot_upcoming mx-2"></span>
                                 <?php } ?>
                                 <span><?= $content_details['data']['genres']??''; ?></span></p>
                              <?php } ?>
                        ...
                        <div class="col-md-4 py-3">
                           <div class="d-flex align-items-center liveev_flex justify-content-end">
                               <?php if ($this->session->id && ($videoType != 'live events')) { ?>
                                 <span class="share_btn_icon me-3 likeAudio liveVideo tooltip-text" tooltip="<?= $this->lang->line("favourite_lang") ?>">
                                    <a href="javascript:void();" data-toggle="modal" show-id="<?= $content_details['data']['id']; ?>" data-target="#share_btn">
        ...
        <?php
   //  pre($content_details['data']['similar']);
   if (isset($content_details) && $content_details['status'] == true && !empty($content_details['data'])) { ?>
         ...
         <div class="container-fluid">
            <div class="row">
               <div class="col-lg-12 mx-auto">
                  <div class="w-100">
                     <?php if (isset($content_details['data']['similar']) && !empty($content_details['data']['similar'])) { ?>
                        <h4 class="text-white m-0 liveVideoHead"><?= $this->lang->line("similar_channel") ?></h4>
                                 ...
                                 <a href="<?= site_url('pb_live_details?id=' . $id); ?>">
                                    <div class="pb_card">
                                       <div class="pb_img pb_l_img">
                                          <img src="<?= $res['thumbnail']??base_url('assets/images/placeholder-poster-img.png'); ?>" class="img-fluid as4" alt="<?=$res['title']?>">
                                       </div>
      ...
      <?php if(isset($content_details['data']['upcoming_shows']) && !empty($content_details['data']['upcoming_shows'])){ ?>
                     ...
                     <div class="w-100">
                        ...
                        <div class='upcoming_data'>
                           <div class="carousel_bott upcoming_program_details owl-carousel owl-theme pt-2 liveVedioArrow banner_load_af">
                              <?php foreach($content_details['data']['upcoming_shows'] as $key => $value){ ?>
                                 <?php if($key > 9){ break; } ?>
                                 <div class="item">
      ...
      <?php if(isset($content_details['data']['past_shows']) && !empty($content_details['data']['past_shows'])){ ?>
         <section class="py-4 d-none">
            <div class="container-fluid">
               <div class="row">
                  <div class="col-lg-12 mx-auto">
                     <div class="w-100">
                        ...
                        <div class='past_data'>
                           <div class="carousel_bott upcoming_program_details owl-carousel owl-theme pt-2 liveVedioArrow banner_load_af">
                           <?php foreach($content_details['data']['past_shows'] as $key => $value){ ?>
                              <?php if($key > 9){ break; } ?>   
                              <div class="item">
   ...
   <script>
      var m_e_id = "<?= $content_details['data']['show_id']??''?>"; 
      var m_e_title = "<?=$content_details['data']['channel_name']??''?>";
      var m_e_genres = "<?=$content_details['data']['genres']??'-'?>";
      ...
      $(document).ready(function(){
         ...
         //          $('.pro_gresss').hide();
         //    }, 3500));
         var back_nav_btn = '<div class="d-flex align-items-center"><div class="back-button bactrm" > <i class = "fa fa-chevron-left text-white back-buttonnn"> </i>  &nbsp;&nbsp;<span class="bac-btn-title">'+"<?= $content_details['data']['title']??''; ?>"+'</span> </div></div>';
      ...
      $(document).ready(function(){
         $('.back-button').on('click', function(event){
            ...
            matomoevents("Stop",m_e_id,m_e_title,m_e_genres);
            // queueTrackingDataWithDelay('trackEvent', [livestring, 'Stop', '<?= ($content_details['data']['id']??'N/A') . "/" . ($content_details['data']['title']??'N/A'). $video_types ?>'],10);
            //    queueTrackingDataWithDelay('trackContentInteraction', [livestring + '/' + 'Stop', '<?= ($content_details['data']['id']??'N/A') . "/" . ($content_details['data']['title']??'N/A'). $video_types ?>','<?= ($content_details['data']['genres']??'N/A') ?? "-" ?>'],100);
            //    queueTrackingDataWithDelay('trackContentImpression', ['<?= ($content_details['data']['id']??'N/A') . "/" . ($content_details['data']['title']??'N/A'). $video_types ?>','<?= ($content_details['data']['genres']??'N/A') ?? "-" ?>'],200);      
      ...
      function play_paused(arg) {
         if (arg == true) {
            ...
            var epgId = "<?=($content_details['data']['id']??'N/A')?>"
            if (epgId != 'N/A') {
               matomoevents("Pause",m_e_id,m_e_title,m_e_genres);

               // queueTrackingDataWithDelay('trackEvent', [livestring, 'Pause', '<?= ($content_details['data']['id']??'N/A') . "/" . ($content_details['data']['title']??'N/A'). $video_types ?>'],0);
               // queueTrackingDataWithDelay('trackContentInteraction', [livestring + '/' + 'Pause', '<?= ($content_details['data']['id']??'N/A') . "/" . ($content_details['data']['title']??'N/A'). $video_types ?>','<?= ($content_details['data']['genres']??'N/A') ?? "-" ?>'],100);
               // queueTrackingDataWithDelay('trackContentImpression', ['<?= ($content_details['data']['id']??'N/A') . "/" . ($content_details['data']['title']??'N/A'). $video_types ?>','<?= ($content_details['data']['genres']??'N/A') ?? "-" ?>'],200);      
            }
         } else {
            ...
            if (epgId != 0) {
               matomoevents("Resume",m_e_id,m_e_title,m_e_genres);

               // queueTrackingDataWithDelay('trackEvent', [livestring, 'Resume', '<?= ($content_details['data']['id']??'N/A') . "/" . ($content_details['data']['title']??'N/A'). $video_types  ?>'],0);
               // queueTrackingDataWithDelay('trackContentInteraction', [livestring + '/' + 'Resume', '<?= ($content_details['data']['id']??'N/A') . "/" . ($content_details['data']['title']??'N/A'). $video_types ?>','<?= ($content_details['data']['genres']??'N/A') ?? "-" ?>'],100);
               // queueTrackingDataWithDelay('trackContentImpression', ['<?= ($content_details['data']['id']??'N/A') . "/" . ($content_details['data']['title']??'N/A') . $video_types ?>','<?= ($content_details['data']['genres']??'N/A') ?? "-" ?>'],200);      
            }
         }
      ...
      var token;
      var player;
      <?php if (isset($content_details['data']) && !empty($content_details['data'])) { ?>
         url = "<?= $content_details['data']['file_url'] ?>";
         drm = "<?= $content_details['data']['is_drm_protected'] ?? 0; ?>";
         token = "<?= !empty($content_details['data']['token']) ? $content_details['data']['token'] : null ?>";
      <?php } else if(isset($content_details['data']) && $content_details['data']['file_url']) { ?>
         url = "<?= $content_details['data']['file_url'] ?>";
         drm = 0;
         token = null;
      ...
      async function hitTrackingUrl(url) {
         if(url){
      ...
      async function player_config(url, browser,token=''){
         var impression_url = '';
         var start_url = '';
         var firstquartile_url = '';
         var midpoint_url = '';
         var thirdquartile_url = '';
         var complete_url = '';
         var stop_url = '';
         ...
         var is_live = "<?=isset($content_details['data']['is_live'])?$content_details['data']['is_live']:''?>";
         if(is_live >1){
         ...
         player.getNetworkingEngine().registerRequestFilter((type, request) => {
            if (request.uris[0].includes("mercury.akamaized.net/cm/e.gif") || request.uris[0].includes("mercury.akamaized.net/cm/i.gif")) {
               ...
               const eventValue = url.searchParams.get("event");
               if(["firstquartile", "midpoint", "thirdquartile", "complete", "stop"].includes(eventValue)){
                  throw new shaka.util.Error(
                     shaka.util.Error.Severity.CRITICAL,
                     shaka.util.Error.Category.NETWORK,
                     shaka.util.Error.Code.REQUEST_FILTER_REJECTED
                  );
               }
               if (url.search) {
         ...
         try {
               ...
               if (response.uri.includes('tracking')) {
                  ...
                  try {
                        ...
                        adMetaData.forEach((item)=>{    
                           ...
                           if(trackingEvents){
                              // ["impression", "start", "firstquartile", "midpoint", "thirdquartile", "complete", "stop"]
                              trackingEvents.forEach((item) => {
                                 if (currentTime <= (item.startTimeInSeconds + item.durationInSeconds)) {
                                    
                                    if(item.eventType == 'impression'){
                                       impression_url = item.beaconUrls[0]??'';
                                    }
                                    if(item.eventType == 'start'){
                                       start_url = item.beaconUrls[0]??'';
                                    }
                                    if(item.eventType == 'firstQuartile'){
                                       firstquartile_url = item.beaconUrls[0]??'';
                                    }
                                    if(item.eventType == 'midpoint'){
                                       midpoint_url = item.beaconUrls[0]??'';
                                    }
                                    if(item.eventType == 'thirdQuartile'){
                                       thirdquartile_url = item.beaconUrls[0]??'';
                                    }
                                    if(item.eventType == 'complete'){
                                       complete_url = item.beaconUrls[0]??'';
                                    }
                                    if(item.eventType == 'stop'){
                                       stop_url = item.beaconUrls[0]??'';
                                    }
           ...
           var strmingnot = "<?= $this->lang->line('staming_not') ?>";
            ...
            videoElement.addEventListener('ended', function() {
               var videoid = "<?=$content_details['data']['id']?>";
               if (!player.isLive() ) {
                  video_check(homeKey, video_id);
            ...
            adManager.addEventListener(shaka.ads.Utils.AD_COMPLETE, (e) => {
               // console.log("AD_COMPLETE", complete_url);
               if(complete_url){
                  hitTrackingUrl(complete_url);
                  complete_url = '';
            ...
            (e) => {
               ...
               if(firstquartile_url){
                  hitTrackingUrl(firstquartile_url);
               }
            }
            );
            ...
            adManager.addEventListener(shaka.ads.Utils.AD_STARTED, (e) => {
               ...
               $(".shaka-resolution-button").addClass("shaka-hidden");
            ...
            adManager.addEventListener(shaka.ads.Utils.AD_STOPPED, (e) => {
               ...
               $(".shaka-resolution-button").removeClass("shaka-hidden");
            ...
            adManager.addEventListener(
               ...
               (e) => {
                  if(thirdquartile_url){
                     hitTrackingUrl(thirdquartile_url);
                     thirdquartile_url = '';
                  }
               }
            );
      ...
      function playPauseCallback() {
         ...
         if (video.paused) {
            ...
            // queueTrackingDataWithDelay('trackEvent', [livestring, 'Pause', '<?= ($content_details['data']['id']??'N/A').'/'.($content_details['data']['channel_id']??'N/A').'/'.($content_details['data']['video_id']??'N/A') . "/" . ($content_details['data']['title']??'N/A') . $video_types?>'],0);
            // queueTrackingDataWithDelay('trackContentInteraction', [livestring + '/' + 'Pause', time_date + ' '+ tiem_la  +'/'+'<?= ($content_details['data']['id']??'N/A').'/'.($content_details['data']['channel_id']??'N/A').'/'.($content_details['data']['video_id']??'N/A') . "/" . ($content_details['data']['title']??'N/A') . $video_types?>','<?= ($content_details['data']['genres']??'N/A') ?? "-" ?>'],100);
            // queueTrackingDataWithDelay('trackContentImpression', [time_date + ' '+ tiem_la  +'/'+'<?= ($content_details['data']['id']??'N/A').'/'.($content_details['data']['channel_id']??'N/A').'/'.($content_details['data']['video_id']??'N/A') . "/" . ($content_details['data']['title']??'N/A') . $video_types ?>','<?= ($content_details['data']['genres']??'N/A') ?? "-" ?>'],200);
         }else{
            matomoevents("Play",m_e_id,m_e_title,m_e_genres);

      //       queueTrackingDataWithDelay('trackEvent', [livestring, 'Play', time_date + ' '+ tiem_la  +'/'+'<?= ($content_details['data']['id']??'N/A').'/'.($content_details['data']['channel_id']??'N/A').'/'.($content_details['data']['video_id']??'N/A') . "/" . ($content_details['data']['title']??'N/A') . $video_types ?>'],0);
      //       queueTrackingDataWithDelay('trackContentInteraction', [livestring + '/' + 'Play', time_date + ' '+ tiem_la  +'/'+'<?= ($content_details['data']['id']??'N/A').'/'.($content_details['data']['channel_id']??'N/A').'/'.($content_details['data']['video_id']??'N/A') . "/" . ($content_details['data']['title']??'N/A') . $video_types?>','<?= ($content_details['data']['genres']??'N/A') ?? "-" ?>'],100);
      // queueTrackingDataWithDelay('trackContentImpression', [time_date + ' '+ tiem_la  +'/'+'<?= ($content_details['data']['id']??'N/A').'/'.($content_details['data']['channel_id']??'N/A').'/'.($content_details['data']['video_id']??'N/A') . "/" . ($content_details['data']['title']??'N/A') . $video_types?>','<?= ($content_details['data']['genres']??'N/A') ?? "-" ?>'],200);
                }
      }
   ...
   <script>
      ...
      $('.likeAudio').click(function() {
         ...
         if ($('.notlikebtn').hasClass("d-none")) {
            var epgId = "<?= ($content_details['data']['id']??0) ?>"
            if (epgId != 0) {
            queueTrackingData('trackEvent', [livestring, 'Unfavourite',  '<?= ($content_details['data']['id']??'')."/" .($content_details['data']['title']??'').'/'.'Live'?>']);
            }

            activity = 3;
            tooltip = '<?= $this->lang->line("favourite_lang") ?>';
         } else if (epgId != 0) {
            queueTrackingData('trackEvent', [livestring, 'Favourite',  '<?= ($content_details['data']['id']??'')."/" .($content_details['data']['title']??'').'/'.'Live'?>']);
         }

         $("#overlayonajaxhit").removeClass('d-none');
         if(epgId != 0){
            var data = {
               show_id: "<?= ($content_details['data']['id']??0); ?>",
               enc_id: "<?= $enc_id??($content_details['data']['id']??0) ?>",
               type: "<?= $content_details['data']['media_type']??0; ?>",
               thumbnail: "<?= ($content_details['data']['thumbnail']??''); ?>",
               poster_url: "<?= ($content_details['data']['poster_url']??''); ?>",
               still_live: "<?= $content_details['data']['still_live']??0; ?>"
            }
   ...
   <script>
      ...
      function copy_link() {
         ...
         var tiem_la=  time_format(time*1000);
         var time_date= formatTimestampToDate(time);
        // matomoevents("Share",m_e_id,m_e_title,m_e_genres);
        queueTrackingDataWithDelay('trackEvent', [livestring, "Share", m_e_id+'/'+m_e_title],10);
               queueTrackingDataWithDelay('trackContentInteraction', ["Share"+"/"+livestring , m_e_id+'/'+m_e_title,m_e_genres],100);
               queueTrackingDataWithDelay('trackContentImpression', [m_e_id+'/'+m_e_title,m_e_genres],200);      
            
      //    queueTrackingDataWithDelay('trackEvent', [livestring, 'Share', time_date + ' '+ tiem_la  +'/'+'<?= ($content_details['data']['id']??'N/A').'/'.($content_details['data']['channel_id']??'N/A').'/'.($content_details['data']['video_id']??'N/A') . "/" . ($content_details['data']['title']??'N/A') . $video_types?>'],0);
		//   queueTrackingDataWithDelay('trackContentInteraction', ["Share" + '/' + livestring,'<?= ($content_details['data']['id']??'N/A') . "/" . ($content_details['data']['title']??'N/A')?>','<?= ($content_details['data']['genres']??'N/A') ?? "-" ?>'],100);
      //  queueTrackingDataWithDelay('trackContentImpression', ['<?= ($content_details['data']['id']??'N/A') ."/" .($content_details['data']['title']??'N/A')?>','<?= ($content_details['data']['genres']??'N/A') ?? "-" ?>'],200);
      }
      ...
      //    var urlcheck = "<?//= $_GET['past']??'' ?>";
        var time = '<?= time();?>'
         ...
         var time_date= formatTimestampToDate(time);
         if(lastSegments == 'live'){
         _paq.push(['setCustomDimension', 4, "LiveEvent" ]);
         }
         if(lastSegments != 'live'){
            _paq.push(['setCustomDimension', 4, "EPGLive" ]);

            queueTrackingDataWithDelay('trackEvent', [livestring, 'Selected',m_e_id+"/"+m_e_title],100);
            queueTrackingDataWithDelay('trackEvent', [livestring, 'View',m_e_id+"/"+m_e_title],300);
         }
      //    if(urlcheck == 'past'){
      //       queueTrackingDataWithDelay('trackEvent', ["Page", 'View', 'Past'],100);
      //    }else{
      //       queueTrackingDataWithDelay('trackEvent', [livestring, 'View', time_date + ' '+ tiem_la  +'/'+'<?= ($content_details['data']['id']??'N/A').'/'.($content_details['data']['channel_id']??'N/A').'/'.($content_details['data']['video_id']??'N/A') . "/" . ($content_details['data']['title']??'N/A') . $video_types?>'],100);
      //    }
      })
   ...
   <script>
    ...
    var showID = "<?= $content_details['data']['show_id']??null ?>";
      function manage_like(type, action) {
        ...
        var data = {
         ...
         var titles = "<?= $content_details['data']['id']?>"+"/"+"<?= $content_details['data']['title']?>";
         var des_gener = '';
   ...
   function ManageLikeEvent(actions,titles,des_gener,type){
      titles = m_e_id+"/"+m_e_title;
      des_gener = m_e_genres;
  ...
  function video_check(homeKey, video_id) {
   $.ajax({
      ...
      async function removeBannerData(homeKey, video_id, callback = null) {
            ...
            function matomoevents(type,id,title,genres){
               queueTrackingDataWithDelay('trackEvent', [livestring, type, id+'/'+title],10);
               queueTrackingDataWithDelay('trackContentInteraction', [livestring + '/' + type, id+'/'+title,genres],100);
