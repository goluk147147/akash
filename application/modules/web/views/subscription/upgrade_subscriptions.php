    ...
    <div class="row">
      <div class="col-lg-12">
        ...
        <section class="">
          ...
          <div class="term-image pt-5 term-imht">
                ...
                <div class="row m-0">
                  <div class="col-md-12 m-auto">
                    <div class="term-section">
                      ...
                      <div class="row">
                        <div class="col-md-5 col-sm-12 trem_sub_pad">

                          <div class="plan-dashobard p-lg-1 mb-4">
                            <small class="mb-1 plan_dash-heading"><?= $this->lang->line('current_plan') ?></small>
                            <div class="d-flex justify-content-between">
                              <h4 class="mb-0 text-white"><?= $activePlan['data']['plan']['title']['en'] ?></h4>
                              <div class="d-flex">
                                <h4 class="mb-0 text-white">₹<?= $activePlan['data']['detail']['plan_amount'] ?></h4>
                                <div class="f-18s"><?= ' / ' . $activePlan['data']['detail']['pricing_title'] ?></div>
                                ...
                                <?php if (isset($activePlan['data']['plan']['features'][0]) && !empty($activePlan['data']['plan']['features'][0])) { ?>
                                  <?php foreach ($activePlan['data']['plan']['features'] as $fkey => $fvalue) { ?>
                                    ...
                                    if (preg_match($pattern, $fvalue['value'], $matches)) {
                                      ...
                                      $value = $fvalue['value'];
                                      if ($value == 1 && stripos($fvalue['title'], " ads") !== false) { 
                                        $value = '<img src="' . base_url('assets/images/tick.svg') . '" alt="tick">';
                                      } else if ($value == 0 && stripos($fvalue['title'], " ads") !== false) {
                                        $value = '<img src="' . base_url('assets/images/cross.svg') . '" alt="cross" height="13px">';
                                      ...
                                      <?php
                                        $search1 = "P)"; $search2 = "p)";
                                          if (stripos($fvalue['value'], $search1) !== false || stripos($fvalue['value'], $search2) !== false) {
                                              $value = preg_replace('/\s*\(.*$/', '', $fvalue['value']); 
                                          } 
                                      ?>
                                      <span class="plan_basic_tt"><?= $fvalue['title'] . ' : ' . $value ?></span>
                                    </div>
                                  <?php } ?>
                                <?php } ?>
                                <div class="">
                                  <div class="clearfixBorder my-3"></div>
                                  <div class="d-flex align-items-center col-lg-12 plan_du_font">
                                    <!-- <span class="me-2 checkicon"><i class="fas fa-check"></i></span> -->
                                    <span class="plan_basic_tt"><span class="plan_dur f-500dur"><?= $this->lang->line('duration') ?> : </span><?= date('jS F, Y', $activePlan['data']['detail']['purchase_date']) . ' - ' . date('jS F, Y', $activePlan['data']['detail']['expiry_date']) ?></span>
                                  </div>
                                </div>
                              </div>
                            </div>
                          ...
                          <?php foreach ($subscriptions['data']['plans'] as $key => $value) { ?>
                            <?php
                            if ($value['id'] == $activePlan['data']['detail']['plan_id']) {
                              continue;
                            }
                            ?>
                            <div class="plan-dashobard p-lg-1 mb-4 hide-plan-div d-none" id="<?= str_replace(['@', '#', '!', '*','-','+',' '], '_', $value['title']['en']) ?>">
                              <!-- <small class="mb-1">Current Plan</small> -->
                              <h4 class="mb-0 text-white"><?= $value['title']['en'] ?></h4>

                              <div class="plan-card">
                                <div class="row ">
                                  <?php foreach ($value['features'] as $fkey => $fvalue) { ?>
                                    <?php
                                    $pattern = '/\{\{(\d+)\}\}/';
                                    if (preg_match($pattern, $fvalue['value'], $matches)) {
                                      $value = $matches[1];
                                        ...
                                        <div class="plan-card">
                                          <div class="row ">
                                            <div class="d-flex align-items-center col-lg-6">
                                             ...
                                             <div class="d-flex align-items-center col-lg-6">
                                              <span class="me-2 checkicon"><i class="fas fa-check"></i></span>
                                              <span>20 Live Channels</span>
                                            </div>                                        
                                        
                                            <div class="d-flex align-items-center col-lg-6">
                                              <span class="me-2 checkicon"><i class="fas fa-check"></i></span>
                                              <span>Ads only on Sports, Live </span>
                                            </div>
                                             <div class="d-flex align-items-center col-lg-6">
                                              <span class="me-2 checkicon"><i class="fas fa-check"></i></span>
                                              <span>HD(720p)</span>
                        ...
                        <div class="col-md-7 col-sm-12 p-3">
                          <div class="upgrad_maindiv p-lg-4 p-md-2 upgrade_padd trem_sub_pad1">
                            <?php if (($max_channels > $subs_channel)) { ?>
                              <div class="d-flex justify-content-end text-primary text-pri-color" id="openSelectChannelRadioModal">
                                <span  class="cursor-pointer" data-bs-target="#selectChannelRadio" data-bs-toggle="modal">+ <?= $this->lang->line('add_channel_radio') ?></span>
                              </div>
                            <?php } ?>
                            ...
                            <div class="channelRadioUpgradMainDiv">
                              <div class="d-flex align-items-center justify-content-between">
                                ...
                                <!-- <div class="child-card">
                                  ...
                                  <div class="" data-bs-target="#liveChannal" data-bs-toggle="modal"><?= $this->lang->line('live_tv_channels') ?> <i class="fas fa-chevron-right ms-2"></i></div>
                                </div>
                                <div class="child-card">
                                  <div class="imgchannel">
                                  ...
                                  <div class="" data-bs-target="#radioChannal" data-bs-toggle="modal"><?= $this->lang->line('live_radio_channels') ?> <i class="fas fa-chevron-right ms-2"></i></div>
                                </div> -->

                              </div>
                              <?php if ($max_channels > $subs_channel) { ?>
                                <?php if ($subs_channel > 0) { ?>
                                  <div class="sub_headingmax"><?= $this->lang->line('addonly') ?><?= $subs_channel ?><?= $this->lang->line('channelsout') ?> <?= $max_channels ?>. <?= $this->lang->line('ypuadd') ?> You can add <?= ($max_channels - $subs_channel) ?> <?= $this->lang->line('morechannels') ?></div>
                                <?php } else if ($max_channels > 0) { ?>
                                  <div class="sub_headingmax"><?= $this->lang->line('take') ?> <?= ($max_channels - $subs_channel) ?> <?= $this->lang->line('channels-await') ?></div>
                                <?php } ?>
                              <?php } ?>
                            </div>
                            ...
                            <div class="channelRadioUpgradMainDiv channelrad_pad">
                              <div class="d-flex justify-content-center align-items-center mt-2 mb-4">
                                <div class="m-q-y text-center">
                                  ...
                                  foreach ($subscriptions['data']['plans'] as $plan_data) {
                                    foreach ($plan_data['pricing'] as $pla) {
                                      ...
                                      if ((!in_array($pla['type'], $schemPlans)) && (($pla['is_upgradable'] == 1) || ($pla['id'] == $activePlan['data']['detail']['pricing_id']))) {
                                  ...
                                  ?>
                                      ...
                                      <button class="<?php if ($i == 0) {
                                                        echo 'active';
                                                      } ?> plan_type" id="nav-year-tab" value="<?= str_replace(['@', '#', '!', '*','-','+',' '], '_', $pla['type']) ?>" type="button"><?= $pla['type'] ?></button>
                                  <?php
                                      if ($i == 0) {
                                        $typename = $pla['type'];
                                      }
                                  ...
                                  <!-- <button class="plan_type" id="nav-year-tab" value="Quaterly" type="button">Quaterly</button>
                                                            
                                  <button class="plan_type" id="nav-year-tab" value="Half-Yearly" type="button">Half Yearly</button>
                                                            
                                  <button class="plan_type" id="nav-year-tab" value="Yearly" type="button">Yearly</button>
                                                            
                                  <button class="plan_type active" id="nav-year-tab" value="Monthly" type="button">Monthly</button> -->
                                </div>
                              </div>

                              <div class="d-flex align-items-center justify-content-around">
                                <div class="w-100 mt-2">
                                  <!-- <div class="radio-buttonss justify-content-between"> -->
                                   <div class="plan_cen plan_cenpad">
                                  <?php 
                                  
                                  foreach ($totalPlans as $key => $plan) { 
                                    // $total_plan = 0;
                                    // foreach ($plan as $skey => $splan) {
                                    //   ++$total_plan;
                                    // }
                                    // pre($total_plan);
                                    // $single_plan = "";
                                    // switch($total_plan){
                                    //   case 1:
                                    //     $single_plan = "single_subs_plan";
                                    //     break;
                                    //   case 2:
                                    //     $single_plan = "double_subs_plan";
                                    //     break;
                                    // }
                                    // $total_plan = 0;
                                    ?>
                                    <div id="<?= str_replace(['@', '#', '!', '*','-','+',' '], '_', $key) ?>" class="radio-buttonss <?= (isset($schemPlans[0]) && ($schemPlans[0] != $key)) ? 'd-none' : '' ?>">
                                      <?php foreach ($plan as $skey => $splan) { ?>
                                        <?php if (($splan['pricing']['id'] != $activePlan['data']['detail']['pricing_id']) && ($splan['pricing']['is_upgradable'] != 1)) {
                                          continue;
                                        } ?>
                                        <label class="custom-radio pricing_label <?= ($splan['pricing']['id'] != $activePlan['data']['detail']['pricing_id']) ? 'upd-radio' : '' ?>" data-id="<?= $splan['id'] ?>" data-title="<?= str_replace(['@', '#', '!', '*','-','+',' '], '_', $splan['title']['en']) ?>">
                                          <?php if ($splan['pricing']['id'] == $activePlan['data']['detail']['pricing_id']) { ?>
                                            <input type="radio" name="radio" disabled>
                                          <?php } else if ($splan['pricing']['is_upgradable'] == 1) { ?>
                                            <input type="radio" class="upgrade_plan" name="price-tab-plan" ${isActive} value="<?= $splan['pricing']['id'] ?>">
                                          <?php } 
                                          
                                          $cur_plan_class = "";
                                          if ($splan['pricing']['id'] == $activePlan['data']['detail']['pricing_id']){
                                            $cur_plan_class = "cur_plan";
                                          }
                                          
                                    
                                          ?>
                                          <span class="radio-btn <?=$cur_plan_class;?>"><i class="fas fa-check"></i>
                                            <div class="hobbies-icon">
                                              <div class="mb-0 h5 premium_ic"><?= $splan['title']['en'] ?></div>
                                              <?php if ($splan['pricing']['id'] == $activePlan['data']['detail']['pricing_id']) { 
                                                $active_plan_id = $activePlan['data']['detail']['pricing_id'];
                                                ?>

                                                <div class="h5 d-flex"><i class="fas fa-rupee-sign pe-1"></i> <?= ($splan['pricing']['s_price'] + $splan['pricing']['gst_amount']) ?> <sub style="bottom: -12px;">&nbsp;<?= $splan['pricing']['type'] ?></sub></div>
                                                <div class="u-plan inactive curr_plan ">
                                                  <?= $this->lang->line('current_plan') ?>
                                                </div>
                                              <?php } else if ($splan['pricing']['is_upgradable'] == 1) { ?>
                                                <div class="h5 d-flex">₹ <?= ($splan['pricing']['s_price'] + $splan['pricing']['gst_amount']) ?> <sub style="bottom: -12px;">&nbsp;<?= $splan['pricing']['type'] ?></sub></div>
                                                <div class="u-plan active">
                                              ...
                                              <input type="hidden" class="price" name="price" value="<?= $splan['pricing']['s_price'] ?>">
                                              <input type="hidden" class="price_gst" name="pricess" value="<?= $splan['pricing']['gst_amount'] ?>">
                                              ...
                                              <input type="hidden" class="gst_amount" name="gst_amount" value="<?= $splan['pricing']['new_gst_amount'] ?>">
                                              <input type="hidden" class="s_price" name="s_price" value="<?= $splan['pricing']['new_amount'] ?>">
                                            </div>
                                          </span>
                                        </label>
                                      <?php } ?>
                                  ...
                                  <?php } ?>
                                  <!-- <label class="custom-radio upg_radio">
                                                        <input type="radio" name="radio" disabled>
                                                        <span class="radio-btn upgrad_p no-cursor"><i class="fas fa-check"></i>
                                                          <div class="hobbies-icon w-100 d-flex justify-content-between align-items-center "> 
                                                            <div>
                                                              <h6 class="mb-0">DD Standard</h6>
                                                            <h6>₹299 /Yeary</h6>
                                                          </div>
                                                          <div class="u-plan inactive">
                                                            Current Plan
                                                          </div>
                                                          </div>
                                                        </span>
                                                      ...
                                                      <label class="custom-radio upg_radio">
                                                        ...
                                                        <span class="radio-btn upgrad_p"><i class="fas fa-check"></i>
                                                           <div class="hobbies-icon w-100 d-flex justify-content-between align-items-center "> 
                                                            <div><h6 class="mb-0">DD Premium</h6>
                                                            <h6>₹666 /Yeary</h6>
                                                          </div>
                                                          <div class="u-plan active">
                                                            Upgrade to
                                  ...
                                  <div class="cur_plan_ele">
                                    <form class="custom-radio-form" method="post" action="<?= base_url('razorpost'); ?>">
                                      ...
                                      <input type="hidden" class="channel_id_checkout" name="channel_id" value="<?= join(',', $activeChannels) ?>">
                                    ...
                                    <div class="promo-flex mt-2 d-none">
                                      <div class="text-center cpn-mdl" style="cursor:pointer">
                                        <img src="<?= base_url('assets/images/coupon.svg') ?>" class="coupon-img">

                                        <button type="button" class="btn border-botoom-dot dddd">
                                          <?= $this->lang->line('have_promo') ?> </button>
                                      </div>
                                      <div clas="" id="coupan"></div>
                                    </div>

                                    <?php 

                                    if($is_upgradable == 1){ ?>
                                        <button class="btn ptp mt-4 w-50 m-auto d-block" id="proceed" disabled><?= $this->lang->line('upgrade_subscription') ?></button>
                                    <?php } else { ?>
                                         <button class="btn ptp mt-4 w-50 m-auto d-block" onclick='urls_call("<?= base_url();?>")'><?= $this->lang->line('continue_watching') ?></button>
                                    <?php }
                                    ...
                                    <div class="mt-2 d-none" id="price-info">

                                      <div class="d-flex align-items-center justify-content-center">
                                        <span id="pay-amt"><?= $this->lang->line('you_pay') ?> : ₹ 801 </span> &emsp; <!-- href="#multiCollapseExample1" -->
                                        <a  id="toggleCollapseButton" class="btn text-white" data-bs-toggle="collapsed"  role="button" aria-expanded="true" aria-controls="multiCollapseExample1" tabindex="0"  onkeypress="handleKeyPress(event)" onkeydown="handleKeyDown(event)" onkeyup="handleKeyUp(event)">
                                          <i class="fas fa-chevron-up"></i>
                                        </a>
                                      </div>
                                      <div class=" multi-collapse" id="multiCollapseExample1">
                                        <div class="card card-body bg-bodyCollapse">

                                          <div class="d-flex justify-content-between align-items-center mb-1">
                                            <div id="pay-plan"><?= $this->lang->line('plan_price') ?></div>
                                            <div id="total-amt"> ₹ 1599</div>
                                          </div>
                                          <div class="d-flex justify-content-between align-items-center mb-1">
                                            <div><?= $this->lang->line('rem_amt_curr_plan') ?></div>
                                            <div id="dis-amt"> ₹ 699</div>
                                          </div>
                                          <div class="d-flex justify-content-between align-items-center mb-1 discount-amt d-none">
                                            <div>Discount</div>
                                            <div id="discount-amt"> ₹ 699</div>
                                          </div>
                                          <div class="clearfixBorder my-2"></div>

                                          <div class="d-flex justify-content-between align-items-center mb-1">
                                            <div><?= $this->lang->line('payble_amt') ?></div>
                                            <div id="rem-amt"> ₹ 801</div>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
  ...
  <div class="modal-dialog modal-dialog-centered modal-md">
    <div class="modal-content mc-content">
      <div class="modal-body p-3 ">
        ...
        <div class="mb-2 mt-2">
          <ul class="nav nav-pills justify-content-around radioChannelUl" id="pills-tab" role="tablist" style="background: #262626; margin: 0px -14px;    border: 1px solid #3C3C3C;border-right:none;border-left:none;">
          ...
          <div class="tab-content" id="pills-tabContent">
            ...
            <h6 class=" textRadioChannel mb-3"><?= $this->lang->line('added_lang') ?><span class="cls"><?= $subs_channel ?></span> <?= $this->lang->line('of') ?> <span id="max-limit"><?= $max_channels ?></span> <?= $this->lang->line('livchannel') ?> </h6>
              ...
              <div class="selectchannelRadio mt-2 mb-2">
                ...
                <div style="max-height: 20vw;overflow: auto;">
                  ...
                  <?php foreach ($channels['data']['channels'] as $key => $value) { ?>
                    <div class="channelLinstingMain w-100" data-id="<?= $value['id'] ?>" data-title="<?= $value['title'] ?>" data-type="LiveChannel">
                      <label class="d-flex justify-content-between w-100 align-items-center">
                        <div class="d-flex align-items-center">
                          <div class="img_"><img src="<?= $value['poster_url'] ?>" alt="poster"></div>
              ...
              <div class="selectchannelRadio mt-2 mb-2">
                <div style="max-height: 20vw;overflow: auto;">
                  <?php foreach ($channels['data']['radio'] as $key => $value) { ?>
                    <div class="channelLinstingMain w-100" data-id="<?= $value['id'] ?>" data-title="<?= $value['title'] ?>" data-type="Radio">
                      <label class="d-flex justify-content-between w-100 align-items-center">
                        <div class="d-flex align-items-center">
                          <div class="img_"><img src="<?= $value['poster_url'] ?>" alt="logos"></div>
                          <div class="channelName">
                            <h4 class="mb-0"><?= $value['title'] ?></h4>
                            <!-- <span class="channelCat">News</span> -->
                          </div>
                        ...
                        <?php foreach ($activePlan['data']['channels'] as $key => $value) {
                          ...
                          <div class="searcSection">
                            <div class="channelfullList">
                              <a href="<?= base_url('pb_live_details?id=') . $id ?>">
                    ...
                    <?php } else { ?>
                      <div class="col-md-6 m-auto text-center">
                        <div class="no_dt_found categaryNo">
                          ...
                          <h5 class="mt-2 text-center text-white" style="margin-top: 20px !important;font-size: 22px;"><?= $this->lang->line('welcome') ?></h5>
    ...
    <div class="modal-content" style="    height: -webkit-fill-available;">
      ...
      <div class="modal-body" style="background: rgba(0, 0, 0, 1)">
        <div class="d-flex align-items-center search_md mt-4">
          <a href="javascript:void(0);" class="pb_back d-flex align-items-center" data-bs-dismiss="modal">
            <i class="fa fa-chevron-left text-white"></i>
            <h4 class="defaultColr mb-0 ms-4 text-white"><?= $this->lang->line('radio_channels') ?></h4>
          </a>
...
<script>
  var cur_plan_id = Number("<?=$active_plan_id;?>");
  var youpay = "<?= $this->lang->line('you_pay') ?>";

  $('.plan_cenpad div.radio-buttonss').each(function() {
        let div_id = $(this).attr('id'); 
        //alert(div_id);
        let labelCount = $('#'+div_id+'.radio-buttonss label').length;
        if(labelCount == 1){
          $('#'+div_id+'.radio-buttonss label').addClass('single_subs_plan');
        } else if(labelCount == 2){
          $('#'+div_id+'.radio-buttonss label').addClass('double_subs_plan');
        }
    });
  

  var channelIdsString; // Declare channelIdsString variable outside event handlers
  $('#liveChannal').on('shown.bs.modal', function() {
    matomoEventsTracker('Page', 'View', 'LiveTVChannels', 6);
  });

  $('#radioChannal').on('shown.bs.modal', function() {
    matomoEventsTracker('Page', 'View', 'LiveRadioChannels', 6);
  });
      ...
      success: function(res) {
        try {
          ...
          if (res.status) {
            ...
            matomoEventsTracker('MyChannels', 'SelectChannel&Radio', 'Save', 6);
            setTimeout(() => {
              location.reload();
            }, 1000);
          } else {
  ...
  $(".plan_type").on('click', function() {
    ...
    $('.radio-buttonss:not(.d-none) .custom-radio.upd-radio:first').trigger('click');
    $('.radio-buttonss:not(.d-none) .cur_plan:first').trigger('click');
    //$('.radio-buttonss:not(.d-none) .custom-radio.upd-radio:first').trigger('click');
  });

  $('.custom-radio').on('click', function() {
    var plan_idd = Number($(this).find('.id').val());
    //alert(plan_idd);
    if(cur_plan_id == plan_idd){ //alert("if");
      $('.cur_plan_ele').addClass('d-none');
    } else{  //alert("else");
      $('.cur_plan_ele').removeClass('d-none');
    }
    ...
    if (is_upgrade) {
      $('.discount-amt').addClass('d-none');
      $('.dddd').show();
      $("#promoInput").val('');
      $('#proceed').attr('disabled', false);
      var price = Number($(this).find('.price').val());
      var gst_price = Number($(this).find('.price_gst').val());
      var gst_amount = Number($(this).find('.gst_amount').val());
      var newAmt = Number($(this).find('.s_price').val());
      // console.log(gst_amount, 'sssssssssss');
      // console.log(newAmt, 'sssssssssss');
      $('#total-amt').html('₹ ' + (price + gst_price).toFixed(2));
      $('#dis-amt').html('₹ ' + ((price + gst_price) - (newAmt + gst_amount)).toFixed(2));
      $('#pay-amt').html(youpay + ' : ₹ ' + (newAmt + gst_amount).toFixed(2));
  ...
  $(document).on('click', '#proceed', function() {
    ...
    queueTrackingData('trackEvent', ["CurrentPlanDetail", "Select", "UpgradeSubscription"]);
  ...
  $(document).on('change', 'input.channel-checkbox', function() {
    ...
    if ($(this).prop('checked')) {
      $('.textRadioChannel .cls').text(len)
      $('.addedmain').prepend(`<div class="added-channel" id="prav${current}" data-id="${current}">
          <img src="${url}" alt="urldeatil">
  ...
  $(document).ready(function() {
    ...
    $(".applyCodeActive").click(function() {
      ...
      channelIdsString = selectedChannels.join(',');
       ...
       if (!channelIdsString || channelIdsString) {
        //console.log(channelIdsString, 'channelIdsString');
        $.ajax({
          ...
          success: function(response) {
            ...
            if (res.status == true) {
              ...
              var pay = Number(res.data.final_mrp) + Number(res.data.final_gst_amount)
              ...
              $('.discount-amt').removeClass('d-none');
              $('#pay-amt').html(youpay + ' : ₹ ' + pay);
              $('#rem-amt').html('₹ ' + pay);
  ...
  $(document).on('click', '.remove', function() {
    ...
    var price = $("#" + pid + " input[name=price-tab-plan]:checked");
    var s_price = $(price).parent('.custom-radio').find('.s_price').val();
    var gst_price = $(price).parent('.custom-radio').find('.gst_amount').val();
    var final_price = parseFloat(s_price) + parseFloat(gst_price);
    //alert(gst_price); alert(final_price);
    $('#final_mrp').val(0);
    $('#final_gst').val(0);
    $('.couponApplied').val(0);
    $('#coupan').text("");
    $('.coupan').hide();
    $('.discount-amt').addClass('d-none');
    $('#pay-amt').html(youpay + ' : ₹ ' + final_price);
    $('#rem-amt').html('₹ ' + final_price);
    $('.dddd').show();
    $("#promoInput").val('');
...
<script>
  var slct_chnl = [];
$(document).ready(function() {
    $('#openSelectChannelRadioModal').on('click', function() {
      // alert("dwwd");
      matomoEventsTracker('MyChannels', 'Select', 'AddMore', 6);
      matomoEventsTracker('MyChannels', 'Select', 'LiveChannel&RadioPopup', 6);
    });
    $('.channelLinstingMain').on('click', function() {
      ...
      if (!slct_chnl.includes(id)) {
        ...
        matomoEventsTracker(type, 'Select', "'" + id + "/" + title + "'", 6);
      }
    });
});
...
$(window).on('load', function() {
  var type = "<?=$type??'' ?>";
    queueTrackingData('trackEvent', ['Page', 'View', 'UpgradeSubscription']);
  })
