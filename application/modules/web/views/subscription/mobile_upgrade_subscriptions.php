...
<style>
  ...
  .bg-black {
    background-color: rgba(0,0,55,1) !important;
  }
  ...
  .plan-name-tab label.active {
    background: linear-gradient(180deg, #5f5aef 0%, #302e9b 100%);
    color: #fff;
  }
...
<section id="main-content">
  ...
  <div class="wrapper site-min-height container-fluid">
    <div class="row m-0">
      <div class="col-lg-12">


        <section class="termCondition" style="margin-bottom: 46vh">

          <div class="term-image pt-5">
            <div class="back-btn pe-4 mb-3">
              <a onclick="history.go(-1)"><i class="fas fa-arrow-left"></i></a>
            </div>
            ...
            ?>
                ...
                <button class="d-none <?php if ($i == 0) {
            ...
            <?php foreach ($totalPlans as $key => $plan) { ?>
              ...
              <div id="<?= str_replace(['@', '#', '!', '*','-','+',' '], '_', $key) ?>" class="  plan-name-tab <?= ($schemPlans[0] != $key) ? 'd-none' : '' ?>">
                <?php foreach ($plan as $skey => $splan) {
                  ...
                  if ($splan['pricing']['is_upgradable'] == 1 && $splan['pricing']['is_active'] == 0) {
                ?>
                    <label class="custom-radio position-relative plan_type <?= ($k == 0) ? 'f-tile' : '' ?> customRadio_<?= $splan['id'] ?>" data-id="<?= $splan['id'] ?>" data-title="<?= str_replace(['@', '#', '!', '*','-','+',' '], '_', $splan['title']['en']) ?>" for="IDRadio_<?= $splan['pricing']['id'] ?>">
                      <?= $splan['title']['en'] ?>
            ...
            <?php foreach ($subscriptions['data']['plans'] as $key => $value) { ?>
              <div class="py-3 bg-black hide-plan-div <?= ($j == 0) ? '' : 'd-none' ?>" id="<?= str_replace(['@', '#', '!', '*','-','+',' '], '_', $value['title']['en']) ?>">
                    ...
                    <?php foreach ($value['features'] as $key =>  $fvalue) { ?>
                      <div class="d-flex align-items-center col-lg-12">
                        ...
                        <?php } else {
                          ...
                          // if ($fvalue['value'] == 1 && stripos($fvalue['title'], " ads") !== false) { 
                          //   //$fvalue['value'] = '<img src="' . base_url('assets/images/tick.svg') . '" alt="tick">';
                          //   $fvalue['value'] = "Yes";
                          // } else if ($fvalue['value'] == 0 && stripos($fvalue['title'], " ads") !== false) {
                        ...
                        foreach ($fvalue as $item) {
                          if (strpos($item, '{{') !== false) {
                            ...
                            <span class="text-primary cursor-pointer add-btn  btn_<?= $planids[$k] ?? '' ?>" data-bs-target="#selectChannelRadio" data-bs-toggle="modal" data-limit="<?= $max_channels ?>">+<?= $this->lang->line('add') ?></span>
                ...
                <div class="main-container mt-3 main_type_button">
                  <div class="bg-black d-f lex justify-content-center align-items-center">
                    <div class="mWeb text-center">
                      <?php $i = 0;
                      foreach ($value['pricing'] as $pricee) {
                        if ($pricee['is_upgradable'] == 1) {   ?>

                          <button class="<?php if ($i == 0) {
                                            echo 'active';
                                          } ?> plan_type_myq " data-price="<?= ($pricee['new_amount'] + $pricee['new_gst_amount']) ?>" data-plan_id="<?= $pricee['plan_id'] ?>" data-pricing_id="<?= $pricee['id'] ?>" data-gst_amount="<?= $pricee['new_gst_amount'] ?>" data-s_price="<?= $pricee['new_amount'] ?>" data-validity="<?= $pricee['validity'] ?>" data-mrp="<?= $pricee['mrp'] ?>" id="nav-year-tab" value="<?= str_replace(['@', '#', '!', '*','-','+',' '], '_', $pricee['type']) ?>" type="button"><?= $pricee['type'] ?></button>
    ...
    <div class="col-md-12 col-sm-12 m-auto p-0">
        ...
        <div class="my-4 d-f lex justify-content-center align-items-center">
          <div class="mWeb text-center">
            ...
            ?>
                ...
                <button class="<?php if ($i == 0) {
                                  echo 'active';
                                } ?> plan_type_myq " data-id="<?= $planids[0] ?? '' ?>" id="nav-year-tab" value="<?= str_replace(['@', '#', '!', '*','-','+',' '], '_', $pla['type']) ?>" type="button"><?= $pla['type'] ?></button>
            <?php if ($i == 0) {
                  $typename = $pla['type'];
                }
                $i++;
      ...
      <div class="main-container mt-3 ">
        ...
        <div class="w-100 d-flex justify-content-between align-items-center">
          <div>
            <div class="f-10up">You Pay</div>
            <div>
              <?php
              $mm = $subscriptions['data']['plans'][0]['pricing'][0]['new_amount'];

              $gst = $subscriptions['data']['plans'][0]['pricing'][0]['new_gst_amount'];
              $merge = $mm + $gst;


              ?>
              <span class="pay_amount"><?= $merge ?></span>
              <small><del><span class="total_amount ms-1"><?= $subscriptions['data']['plans'][0]['pricing'][0]['mrp'] ?></span></del></small>
              <span class="pay_pric"><i class="fas fa-chevron-up ms-3"></i></span>
            </div>

          </div>
          <?php if($is_upgradable == 1){ ?>
            <button class="btn w-50 ptp mt-3" id="proceed"><?= $this->lang->line('upgrade_subscription') ?><span class="proceed"></span></button>
          <?php } else { ?>
            <button class="btn w-50 ptp mt-3" onclick='urls_call("<?= base_url();?>")'><?= $this->lang->line('continue_watching') ?></button>
          <?php }
          ?>
        </div>
      </div>
...
<div class="modal fade bd-example-modal-sm " id="pay_ammount" tabindex="-1" role="dialog" aria-labelledby="pay_ammount" aria-hidden="true">
  <div class="modal-dialog modal_sm modal-dialog-custom" style="bottom: -10px; position: fixed;">
    <div class="modal-content mc-content" style="width:100vw">
      <div class="modal-body p-3 ">

        <div class="d-flex align-items-center justify-content-between mb-3" style="padding: 4px 0">
          <span class="f-16s"><?= $this->lang->line('price_details') ?></span>
          <span class="Crossmodal cross_modal_dt closecross" data-bs-dismiss="modal"> <img src="<?= base_url('assets/images/sunscription_close.svg'); ?>" class="subscription_close_img" alt="subscription_close"></span>
        </div>
        <div class="clearfixBorder my-3"></div>

        <div class="d-flex justify-content-between align-items-center mb-2">
          <div class="w-50"><?= $this->lang->line('plan_price') ?></div>
          <div class="w-50 text-right"><span class="remaining_payable_amount">₹ <?= $subscriptions['data']['plans'][0]['pricing'][0]['mrp'] ?></span></div>
        </div>
        <div class="d-flex justify-content-between align-items-center mb-2">
          <div class="w-50"><?= $this->lang->line('rem_amt_curr_plan') ?></div>
          <div class="w-50 text-right"><span class="remaining_amount">₹ </span><?= $subscriptions['data']['balance_detail']['balance'] ?></div>
        </div>
        <?php
        $mm = $subscriptions['data']['plans'][0]['pricing'][0]['new_amount'];
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
          <div class="w-50"><?= $this->lang->line('payble_amt') ?></div>
          <div class="w-50 text-right"><span class="remaining_amount_pay_u">₹ <?= $merge ?> </span></div>
        </div>
...
<form class="custom-radio-form" method="post" action="<?= base_url('razorpost'); ?>">
  ...
  <input type="hidden" class="gst_amount_checkout" id="gst_amount" name="gst_amount" value="<?= $subscriptions['data']['plans'][0]['pricing'][0]['new_gst_amount'] ?>">
  <input type="hidden" class="s_price_checkout" id="s_price" name="s_price" value="<?= $subscriptions['data']['plans'][0]['pricing'][0]['new_amount'] ?>">
      ...
      <div class="modal-body p-3 ">
        ...
        <div class="mb-2 mt-2">
          ...
          <div class="tab-content" id="pills-tabContent">
            ...
            <h6 class=" textRadioChannel mb-3">Added <span class="cls"><?= $subs_channel ?></span> of <span id="max-limit"><?= $max_channels ?></span> Live Channels </h6>
...
<script>
  ...
  $(document).on('click', '.add_more_chanel', function() {
    var maxchnle = $(this).attr('data-max_cahnnel');

    $("#selectChannelRadio").modal('show');
  });
  ...
  $(".plan_type_myq").on('click', function() {
    ...
    $('.total_amount').text(mrp);
    // $('.remaining_amount').text(s_price);
    $('.remaining_payable_amount').text(mrp);
    $('.remaining_amount_pay_u').text(price);
  ...
  $('.pay_pric').on('click', function() {

    $('#pay_ammount').modal('show');
  });
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          if (res.status == true) {
            ...
            // $(".proceed").text(' - ₹ ' + (final_mrp + ggg));
            $('#final_gst').val(ggg);
            $('.couponApplied').val(res.data.coupon_id);
            $('.remaining_amount_pay_u').text(final_mrp + ggg);
            $('.pay_amount').text(final_mrp + ggg);
  ...
  $(document).on('click', '.remove', function() {
    ...
    var price = $(".plan_type_myq.active").attr('data-price');
