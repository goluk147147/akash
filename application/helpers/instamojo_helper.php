...
if (!function_exists('paymentRequestCreate')) {
    ...
    {
        // print_r('dfghj');die;
        $response = api_call('POST', 'payment-requests', $payment_request); 
...
//     */
     function build_curl_headers() 
    {
        $headers = array("X-Api-key: 'test_307e610274a17e5e1db7cd783f0'");
        // if($this->auth_token) {
...
//     */
     function build_api_call_url($path)
    {
        if (strpos($path, '/?') === false and strpos($path, '?') === false) {
            return 'https://test.instamojo.com/api/1.1/' . $path . '/';
        }
        return 'https://test.instamojo.com/api/1.1/' . $path;
...
// if(!function_exists(api_call)){
     function api_call($method, $path, array $data=null) 
    {
        ...
        $data = (array) $data;
        $headers = build_curl_headers();
        // print_r($headers);die;
        $request_url = build_api_call_url($path);
