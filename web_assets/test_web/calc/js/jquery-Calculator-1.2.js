/*********************************************************************************************/
// $Calculator v1.2 || Crusader 12 || Exclusive to CodeCanyon
/*********************************************************************************************/
(function ($) {
    var z_index = 9998;
    var Calculator = {
        ...
        defaults: {
            ...
            type: 'standard', // TYPE OF Calculator CALCULATOR (STANDARD OR MINI)
            solar: true, // DISPLAY SOLAR PANELS
            title: 'Calculator', // TITLE OF CALCULATOR
...
// INITIALIZE
        ...
        init: function (options) {
            ...
            $(document).off('click.Calculator touchstart.Calculator').on('click.Calculator touchstart.Calculator', function (e) {
                var $target = $(e.target);
                // CLOSE ALL CALCS
                Calculator.closeCalcs($target);
            });
            ...
            return this.each(function () {
                ...
                var $this = $(this),
                        o = options,
                        mergedData = $.extend({}, Calculator.defaults, o),
                        calcD = $this.data('Calculator'),
                        cD = calcD != undefined ? calcD : false;
                $.data($this, $.extend({}, mergedData, !cD ? {} : cD || {}));
                $this.data('Calculator', $.data($this));
                var noD = $this.data('Calculator');
                noD.AniStat = 0;

                // BUILD HTML
                Calculator.buildHTML($this, noD);

                // SETUP CALCULATOR DRAGGABILITY
                if (noD.drag)
                    Calculator.addDrag($this, false);
                ...
                if (noD.type.removeWS().toLowerCase() === 'standard') {
                    ...
                    //let z_index = noD.topIndex.pF();
                    $this.css({'z-index': z_index, top: '10%', left: '30%'}).find('input[type=text]').add('input.no_oldresult').val('');
                    ...
                    $this.on('mouseup touchend', function (e) {
                        ...
                        switch ($key) {
                            case $calc.find('input.key_1')[0] 		:
                                Calculator.cdef($calc, 1);
                                break;
                            case $calc.find('input.key_2')[0]		:
                                Calculator.cdef($calc, 2);
                                break;
                            case $calc.find('input.key_3')[0]		:
                                Calculator.cdef($calc, 3);
                                break;
                            case $calc.find('input.key_4')[0]		:
                                Calculator.cdef($calc, 4);
                                break;
                            case $calc.find('input.key_5')[0] 		:
                                Calculator.cdef($calc, 5);
                                break;
                            case $calc.find('input.key_6')[0] 		:
                                Calculator.cdef($calc, 6);
                                break;
                            case $calc.find('input.key_7')[0]		:
                                Calculator.cdef($calc, 7);
                                break;
                            case $calc.find('input.key_8')[0]		:
                                Calculator.cdef($calc, 8);
                                break;
                            case $calc.find('input.key_9')[0] 		:
                                Calculator.cdef($calc, 9);
                                break;
                            case $calc.find('input.key_0')[0] 		:
                                Calculator.cdef($calc, 0);
                                break;
                            case $calc.find('input.key_pi')[0] 		:
                                Calculator.cdef($calc, Math.PI);
                                break;
                            case $calc.find('input.key_decimal')[0] 	:
                                Calculator.cdef($calc, '.');
                                break;
                            case $calc.find('input.key_add')[0] 		:
                                Calculator.cdef($calc, '+');
                                break;
                            case $calc.find('input.key_e')[0] 		:
                                Calculator.cdef($calc, 'e');
                                break;
                            case $calc.find('input.key_minus')[0] 	:
                                Calculator.cdef($calc, '-');
                                break;
                            case $calc.find('input.key_multiply')[0] :
                                Calculator.cdef($calc, '*');
                                break;
                            case $calc.find('input.key_divide')[0] 	:
                                Calculator.cdef($calc, '/');
                                break;
                            case $calc.find('input.key_parr')[0] 	:
                                Calculator.cdef($calc, ')');
                                break;
                            case $calc.find('input.key_parl')[0] 	:
                                Calculator.cdef($calc, '(');
                                break;
                            case $calc.find('input.key_enter')[0] 	:
                                Calculator.ghij($calc, 1);
                                break;
                            case $calc.find('input.key_square')[0] 	:
                                Calculator.ghij($calc, 2);
                                break;
                            case $calc.find('input.key_squareroot')[0]:
                                Calculator.ghij($calc, 3);
                                break;
                            case $calc.find('input.key_changesign')[0]:
                                Calculator.ghij($calc, 4);
                                break;
                            case $calc.find('input.key_natlog')[0] 	:
                                Calculator.ghij($calc, 5);
                                break;
                            case $calc.find('input.key_natanti')[0] 	:
                                Calculator.ghij($calc, 6);
                                break;
                            case $calc.find('input.key_1x')[0] 		:
                                Calculator.ghij($calc, 7);
                                break;
                            case $calc.find('input.key_log')[0] 		:
                                Calculator.ghij($calc, 8);
                                break;
                            case $calc.find('input.key_antil')[0] 	:
                                Calculator.ghij($calc, 9);
                                break;
                            case $calc.find('input.key_atan')[0] 	:
                                Calculator.ghij($calc, 10);
                                break;
                            case $calc.find('input.key_acos')[0]		:
                                Calculator.ghij($calc, 11);
                                break;
                            case $calc.find('input.key_asin')[0] 	:
                                Calculator.ghij($calc, 12);
                                break;
                            case $calc.find('input.key_tan')[0] 		:
                                Calculator.ghij($calc, 14);
                                break;
                            case $calc.find('input.key_cos')[0] 		:
                                Calculator.ghij($calc, 15);
                                break;
                            case $calc.find('input.key_sin')[0] 		:
                                Calculator.ghij($calc, 16);
                                break;
                            case $calc.find('input.key_percent')[0] 	:
                                Calculator.ghij($calc, 17);
                                break;
                            case $calc.find('input.key_ppm')[0] 		:
                                Calculator.ghij($calc, 18);
                                break;
                            case $calc.find('input.key_x')[0] 		:
                                Calculator.ghij($calc, 20);
                                break;
                            case $calc.find('input.key_xy')[0] 		:
                                Calculator.ghij($calc, 21);
                                break;
                            case $calc.find('input.key_root')[0] 	:
                                Calculator.ghij($calc, 22);
                                break;
                            case $calc.find('input.key_MS')[0] 		:
                                Calculator.memory($calc, 1);
                                break;
                            case $calc.find('input.key_MR')[0] 		:
                                Calculator.memory($calc, 2);
                                break;
                            case $calc.find('input.key_cls')[0] 		:
                                Calculator.memory($calc, 3);
                                break;
                            case $calc.find('input.key_backspace')[0] :
                                Calculator.backspace($calc);
                                break;
                            case $calc.find('label.link_rad')[0] :
                                $this.find('input.key_radians').click();
                                e.preventDefault;
                                break;
                            case $calc.find('label.link_grad')[0]:
                                $this.find('input.key_gradient').click();
                                e.preventDefault();
                                break;
                            case $calc.find('label.link_deg')[0] :
                                $this.find('input.key_degree').click();
                                e.preventDefault();
                                break;
                            case $calc.find('.key_radians')[0] || $('.key_degree')[0] || $('.key_gradient')[0] :
                                $this.focus();
                                break;
                        }
                        ;
                    ...
                    $this.find('input.no_results').on('focus', function (e) {
                        Calculator.display($this, $(this).val());
                        e.preventDefault;
                    });
                    ...
                    $this.find('select.no_dec_select').on('change', function () {
                        ...
                        if ($oldR.val() !== '')
                            Calculator.result($calc, $oldR.val());
                    ...
                    $this.find('div.no_rb').on('mouseover touchstart', function (e) {
                        switch (e.target) {
                            case $('input.link_rad')[0]:
                                self.status = 'Radians';
                                e.preventDefault();
                                break;
                            case $('input.link_deg')[0]:
                                self.status = 'Degrees';
                                e.preventDefault();
                                break;
                            case $('input.link_grad')[0]:
                                self.status = 'Gradients';
                                e.preventDefault();
                                break;
                        }
                        ;
                    ...
                    $this.find('input.no_calc_field').on('keypress', function (e) {
                        ...
                        if (this.value.match(/^[a-zA-Z]+$/)) {
                            ...
                            if (e.keyCode == 13)
                                Calculator.ghij($this.parents('form.night_owl_calc'), 1);
                            return true;
                        }
                        ;
                        ...
                        $this.find('input.no_key_toggle')
                                .on('mouseup touchend', function (e) {
                                    var $calc = $(this).parents('form.night_owl_calc:first'),
                                            noD = $calc.parent().data('Calculator'),
                                    ...
                                    if (noD.mode === 'basic') {
                                        ...
                                        $sciKeys.stop(true, false).animate({opacity: 1}, {duration: noD.mode_switch_speed.pF(), queue: false, complete: function () {
                                                ...
                                                if (noD.mode !== 'scientific' && noD.assign_field && !noD.pos_moved)
                                                    Calculator.positionCalc($calc.parent(), $calc.parent().prev($('.' + noD.assign_field)), noD);
                                                noD.mode = 'scientific';
                                            }});
                                        // IN SCIENTIFIC MODE
                                    } else {
                                        ...
                                        $sciKeys.stop(true, false).animate({opacity: 0}, {duration: noD.mode_switch_speed.pF(), queue: false, complete: function () {
                                                ...
                                                if (noD.mode !== 'basic' && noD.assign_field && !noD.pos_moved)
                                                    Calculator.positionCalc($calc.parent(), $calc.parent().prev($('.' + noD.assign_field)), noD);
                                                noD.mode = 'basic';
                                            }});
                                    }
                    ...
                    switch (noD.equation.toLowerCase().removeWS()) {
                        case 'velocity'     :
                            noD.equation = "Velocity[m/s]=Distance[meters] / Time[seconds]";
                            break;
                        case 'acceleration' :
                            noD.equation = "Acceleration[m/s]=( Final_Speed[m/s] - Initial_Speed[m/s] ) / Time[seconds]";
                            break;
                        case 'momentum' :
                            noD.equation = "Momentum[Ns]=Mass[kg] * Velocity[m/s]";
                            break;
                        case 'bmi':
                            noD.equation = "BMI=( Weight[lbs] / ( ( ( Height_Feet[ft] * 12 ) + Height_Inches[inches] ) * ( ( Height_Feet[ft] * 12 ) + Height_Inches[inches] ) ) ) * 703";
                            break;
                        case 'kinetic_enery':
                            noD.equation = "Kinetic_Energy[KE]=0.5 * Mass[kg] * Velocity[m/s] ^^ 2";
                            break;
                        case 'potential_energy':
                            noD.equation = "Potential_Enery[PE]=Mass[kg] * Acceleration_of_Gravity[m/s^2] * Height[meters]";
                            break;
                        case 'depreciation':
                            noD.equation = "Period_Depriciation[$/Years]=( Initial_Cost[$] - Salvage_Value[$] ) / Useful_Years";
                            break;
                        case 'simple_interest':
                            noD.equation = "Simple_Interest=Principal[$] * ( Interest_Rate[%] / 100 ) * Number_of_Periods";
                            break;
                        case 'compound_interest' :
                            noD.equation = "Amount_Accumulated[$]=Principal[$] * |( 1 + ( ( Interest_Rate[%] / 100 ) / Compoundings_per_Year ) )| ^^ ( Compoundings_per_Year * Number_of_Years )";
                            break;
                        case 'proportion':
                            noD.equation = "Proportionate Value=( Value_A * Value_C ) / Value_B";
                            break;
                        case 'ounces_cups' :
                            noD.equation = "Cups=Ounces[oz] / 8";
                            break;
                        case 'fahrenheit_celsius':
                            noD.equation = "Degrees[C]=( ( Degrees[F] - 32 ) * 5 ) / 9";
                            break;
                        case 'celsius_fahrenheit':
                            noD.equation = "Degrees[F]=( ( Degrees[C] * 9 ) / 5 ) + 32";
                            break;
                        case 'force'		:
                            noD.equation = "Force[N]=Mass[kg] * Acceleration[(m/s)^2]";
                            break;
                        case 'horsepower'	:
                            noD.equation = "Horsepower[Watt]=Torque[pound/ft] * ( Speed[rpm] / 5252)";
                            break;
                        case 'centripetal_force':
                            noD.equation = "Centripetal Force[F]=( Mass[kg] * Velocity[m/s]^2 ) / Circular_Radius[meters]";
                            break;
                        case 'discount' :
                            noD.equation = "Sale_Price[$]=Cost[$] - ( Cost[$] * ( Discount[%] / 100 ) )";
                            break;
                        case 'markup' :
                            noD.equation = "Markup_Price[$]=Trade_Cost[$] + ( Trade_Cost * ( Desired_Markup[%] / 100 ) )";
                            break;
                        case 'area_triangle' :
                            noD.equation = "Area_of_Triangle=( Length_of_Base * Length_of_Height ) / 2";
                            break;
                        case 'project_cost' :
                            noD.equation = "Project_Cost[$]=( Time[hours] * Hourly_Rate[$] ) + ( Revisions[#] * Revision_Cost[$] )";
                            break;
                    }
                    ;
                    ...
                    $calc.html(html += answer_input + '<input type="button" value="Calculate" class="no_calculate"><input type="button" value="Reset" class="no_reset"></form>')
                        ...
                        var noD = $calc.data('Calculator');
                        $calc.find('input.no_in').add($calc.find('input.no_out')).val('');
                        if (noD.assign_field)
                            $calc.prev($('.' + noD.assign_field)).val('');
                        e.preventDefault();
                    })
                        ...
                        if (isNaN(answer)) {
                            ...
                            if (typeof window[noD.on_solve] === 'function')
                                window[noD.on_solve].apply(Calculator);
                        }
                        ;
                ...
                if (noD.assign_field) {
                    ...
                    $('.' + noD.assign_field).on('click touchstart focus', function (e) {
                        ...
                        Calculator.closeCalcs($el);

                        // OPTIONAL POSITION MATCHING 
                        if (noD.match_pos)
                            Calculator.positionCalc($this, $el, noD);
        ...
        closeCalcs: function ($target) {
            ...
            for (var i = 0, l = $('.active_calc').length; i < l; i++) {
                ...
                if (noD.opened && $target[0] !== $('.' + noD.assign_field)[0]) {
                    ...
                    if ($target.parents('.active_calc').length > 0)
                        return;
...
// FORMAT STRING AS MONEY (THOUSANDS, AND DECIMALS)
        ...
        formatMoney: function (value) {
            return '$' + value.pF().toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
        },
...
// POSITION CALCULATOR
        ...
        positionCalc: function ($calc, $el, noD) {
            ...
            switch (noD.match_pos) {
                case 'right_down' :
                    var top = el_y, left = el_x + el_w;
                    break;
                case 'right_up'   :
                    var top = el_y - calc_h + el_h, left = el_x + el_w;
                    break;
                case 'right_mid'  :
                    var top = el_y - calc_h / 2 + el_h, left = el_x + el_w;
                    break;
                case 'top_left'   :
                    var top = el_y - calc_h, left = el_x;
                    break;
                case 'top_right'  :
                    var top = el_y - calc_h, left = el_x - (calc_w - el_w);
                    break;
                case 'top_mid'     :
                    var top = el_y - calc_h, left = el_x - (calc_w / 2) + (el_w / 2);
                    break;
                case 'bottom_right':
                    var top = el_y + el_h, left = el_x - (calc_w - el_w);
                    break;
                case 'bottom_left' :
                    var top = el_y + el_h, left = el_x;
                    break;
                case 'bottom_mid'  :
                    var top = el_y + el_h, left = el_x - (calc_w / 2) + (el_w / 2);
                    break;
                case 'left_up'    :
                    var top = el_y - calc_h + el_h, left = el_x - calc_w;
                    break;
                case 'left_down'  :
                    var top = el_y, left = el_x - calc_w;
                    break;
                case 'left_mid'   :
                    var top = el_y - calc_h / 2 + el_h, left = el_x - calc_w;
                    break;
            }
            ;
...
// CALCULATOR MEMORY METHOD
        ...
        memory: function ($calc, operator) {
            ...
            if (operator == 1) {
                ...
                if (mem == 0 || Calculator.chracter(mem.charAt(0)))
                    mem = '';
...
// CALCULATOR DISPLAY METHOD
        ...
        display: function ($calc, xyz) {
            if (xyz == '') {
                ...
                $calc.find('input.no_results').select();
            }
            ;
        },
        ...
        ghij: function ($calc, sciKeyNum) {
            ...
            if (sciKeyNum >= 1) {
                if ($calcField.val() == '') {
                    ...
                    val = $calcField.val();
                    if (Calculator.resultant(val.charAt(0)))
                        val = $calc.find('input.no_results').val() + val;
                }
                ;
            }
            ;
            ...
            ;
            if (Calculator.operator(val.charAt(val.length - 1)))
                return false;
            answer = eval('1*' + answer);

            // SCIENTIFIC KEYS
            if (sciKeyNum > 1)
                answer = Calculator.mathcalc($calc, sciKeyNum, answer);
            ...
            if (answer < 0 && $calcField.val() < 0)
                var answer = eval(oldVal + answer);

            // SAVE ANSWER AS OLD ANSWER
            $calc.find('input.no_oldresult').val(answer);

            // DISPLAY RESULT
            Calculator.result($calc, answer);
            $calcField.val('').focus();
        },
...
// CALCULATOR RESULTS METHOD
        ...
        result: function ($calc, screenVal) {
            ...
            var intVal = strVal.length - 1, // REMOVE LAST CHARACTER		
                    noD = $calc.parent().data('Calculator');
            Calculator.decklmn(strVal);

            // DECIMALS
            if (intVal > 16 && ghi == -1) {
                if (decimal == -1)
                    decimal = 14;
                strVal = Calculator.xyzab(strVal.substring(0, intVal)) + ' ';
            ...
            if (decimal >= 0 && decimal != 14) {
                if (def > 0) {
                    var answer = Calculator.xyzab(strVal.substring(0, intVal));
                } else {
                ...
                decimal = 14;
                var answer = Calculator.xyzab(strVal);
            }
            ;
            if (answer.charAt(0) == '.')
                answer = '0' + answer;
            ...
            if (noD.assign_field)
                $calc.parent().prev('.' + noD.assign_field).val(isNaN(answer) ? 'Error' : answer);
        ...
        mathcalc: function ($calc, klmn, mno) {
            with (Math) {
                if (klmn == 2) {
                    ...
                    ;
                    if ($calc.find('input.key_degree')[0].checked)
                        mno = (mno * 180) / PI;
                    ...
                    mno = Calculator.factorial(mno);
                } else if (klmn == 21) {
                    jkl = prompt("Enter Root", 3);
                    mno = pow(mno, jkl);
                } else if (klmn == 22) {
                    jkl = prompt("Enter Root", 3);
                    mno = pow(mno, (1 / jkl));
...
// CALCULATOR HELPER / UTILITY METHODS
        /***********************************************************************************************/
        cdef: function ($calc, xyz) {
            $calc.find('input.no_calc_field').focus()[0].value += xyz;
        },
        factorial: function (n) {
            if ((n == 0) || (n == 1)) {
                ...
                var opqrst = (n * Calculator.factorial(n - 1));
                return opqrst
            }
            ;
        },
        decklmn: function (data1) {
            def = 0;
            ghi = 0;
            def = data1.indexOf(".");
            ghi = data1.indexOf("e");
        },
        resultant: function (valuer) {
            var resultant = '*/+';
...
// BUILD CALCULATOR HTML
        ...
        buildHTML: function ($calc, noD) {
            ...
            // TYPE OF Calculator CALCULATOR TO LOAD
...
// ADD DRAG FUNCTIONALITY TO CALCULATOR
        ...
        addDrag: function ($calc, $handle) {
            ...
            return $el.addClass('no_draggable').on('mousedown', function (e) {
                var $this = $(this),
                        noD = $this.data('Calculator'),
                        $target = $(e.target);

                // DON'T DRAG ON BUTTONS
                if ($target.hasClass('no_calculate') || $target.hasClass('no_reset'))
                    return;

                // SAVE REFERENCE TO ACTIVE CALCULATOR
                Calculator.$activeCalc = $el;
                ...
                var z_idx = $el.css('z-index'),
                        ...
                        tI = $el.data('Calculator').topIndex.pF() + 1;
                $el.css('z-index', z_index).data('Calculator').topIndex = tI;

                $(document).on('mousemove.Calculator', function (e) {
                    if (!Calculator.$activeCalc)
                        return;

                    // CALCULATOR HAS BEEN DRAGGED (DON'T REPOSITION ON TOGGLE)
                    noD.pos_moved = true;

                    Calculator.$activeCalc.offset({top: e.pageY + pos_y - drg_h, left: e.pageX + pos_x - drg_w})
                            .on('mouseup', function () {
                ...
                if (!$target.hasClass('no_in'))
                    e.preventDefault();

            }).on('mouseup', function () {
                var $this = $(this);
                Calculator.$activeCalc = false;
...
// PLUGIN DEFINITION
    /***********************************************************************************************/
    $.fn.Calculator = function (method, options) {
        if (Calculator[method]) {
            return Calculator[method].apply(this, Array.prototype.slice.call(arguments, 1));
        } else if (typeof method === 'object' || !method) {
            return Calculator.init.apply(this, arguments);
        } else {
...
String.prototype.sP = function (splitter, key) {
    return this.toString().split(splitter)[key];
};
